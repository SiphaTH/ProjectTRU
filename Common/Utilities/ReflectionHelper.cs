...
public static class ReflectionHelper
{
    ...
    public static Type GetCachedType(this Assembly assembly, string typeName) => RetrieveFromCache(ReflectionType.Type, typeName, () => assembly.GetType(typeName));

    public static FieldInfo GetCachedField(this Type type, string fieldName) => RetrieveFromCache(ReflectionType.Field, GetFieldNameForCache(type, fieldName), () => type.GetField(fieldName, UniversalFlags));

    public static PropertyInfo GetCachedProperty(this Type type, string propertyName) => RetrieveFromCache(ReflectionType.Property, GetPropertyNameForCache(type, propertyName), () => type.GetProperty(propertyName, UniversalFlags));
    ...
    private static string GetFieldNameForCache(Type type, string fieldName)
    {
    ...
    private static string GetPropertyNameForCache(Type type, string property)
    {
        string assemblyName = type.Assembly.GetName().Name;
        string typeName = type.Name;
    ...
    private static string GetConstructorNameForCache(Type type, params Type[] types)
    {
        string assemblyName = type.Assembly.GetName().Name;
        string typeName = type.Name;
    ...
    public static TFieldType GetPropertyValue<TType, TFieldType>(this TType obj, string property) => (TFieldType)typeof(TType).GetCachedProperty(property)?.GetValue(obj);

    public static void SetPropertyValue<TType, TFieldType>(this TType obj, string property, TFieldType value) => typeof(TType).GetCachedProperty(property)?.SetValue(obj, value);

    public static FieldInfo GetCachedField<TType>(string fieldName) => typeof(TType).GetCachedField(fieldName);
    
    public static MethodInfo GetCachedMethod<TType>(string fieldName) => typeof(TType).GetCachedMethod(fieldName);

    public static PropertyInfo GetCachedProperty<TType>(string propertyName) => typeof(TType).GetCachedProperty(propertyName);

    public static void SetToNewInstance(this FieldInfo field, object fieldInstance = null, object constructedInstance = null) => field.SetValue(fieldInstance, constructedInstance ?? Activator.CreateInstance(field.FieldType));

    public static void SetToNewInstance(this PropertyInfo property, object propertyInstance = null, object constructedInstance = null) => property.SetValue(propertyInstance, constructedInstance ?? Activator.CreateInstance(property.PropertyType));
