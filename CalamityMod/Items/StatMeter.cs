				...
				if (CalamityRuTranslate.TRuConfig.CalamityTranslation)
				{
					if (item.type == calamity.ItemType("StatMeter"))
					{
						foreach (TooltipLine tooltipLine in tooltips)
						{
							...
							string resultC = resultB.Replace("Adrenaline Damage Boost", "Бонус урона адреналина");
							string resultD = resultC.Replace("Adrenaline DR Boost", "Бонус сопротивление урона адреналина");
							string resultE = resultD.Replace("Rage Damage Boost", "Бонус урона ярости");
							string resultF = resultE.Replace("True Melee Damage", "Истинный урон ближнего боя");
							string resultG = resultF.Replace("Melee Damage", "Урон ближнего боя");
							string resultH = resultG.Replace("Melee Crit Chance", "Шанс критического удара ближнего боя");
							string resultI = resultH.Replace("Melee Speed Boost", "Скорость атаки ближнего боя");
							string resultJ = resultI.Replace("Ranged Damage", "Стрелковый урон");
							string resultK = resultJ.Replace("Ranged Crit Chance", "Стрелковый шанс критического удара");
							string resultL = resultK.Replace("Ammo Consumption Chance", "Шанс использовать боеприпасы");
							string resultM = resultL.Replace("Magic Damage", "Магический урон");
							string resultN = resultM.Replace("Magic Crit Chance", "Магический шанс критического удара");
							string resultO = resultN.Replace("Mana Usage", "Расход маны");
							string resultP = resultO.Replace("Mana Regen", "Регенерация маны");
							string resultQ = resultP.Replace("Minion Damage", "Урон миньонов");
							string resultR = resultQ.Replace("Minion Slots", "Миньон-слотов");
							string resultS = resultR.Replace("Rogue Damage", "Разбойный урон");
							string resultT = resultS.Replace("Rogue Crit Chance", "Разбойный шанс критического удара");
							string resultU = resultT.Replace("Rogue Velocity Boost", "Увеличение скорости снарядов разбойника");
							string resultV = resultU.Replace("Rogue Weapon Consumption Chance", "Вероятность потребления разбойного оружия");
							string resultW = resultV.Replace("Max Stealth", "Максимальная скрытность");
							string resultX = resultW.Replace("Standing Regen", "Стоячая регенерация");
							string resultY = resultX.Replace("seconds", "секунд");
							string resultZ = resultY.Replace("Moving Regen", "Подвижная регенерация");
							...
							string resultAp = resultAo.Replace("Other Abyss stats are only displayed while in the Abyss", "Другие характеристики бездны отображаются только во время нахождения в бездне");
