...
namespace CalamityRuTranslate.ThoriumMod.Items
{
    public class ThoriumVanillaItems : GlobalItem
    {
        public override void ModifyTooltips(Item item, List<TooltipLine> tooltips)
        {
            ...
            {
                if (item.type == 1113 || item.type == 1110 || item.type == 1109 || item.type == 1114 || item.type == 1108 || item.type == 1112 || item.type == 1111 || item.type == 1107 || item.type == 1291 || item.type == 183 || item.type == 68 || item.type == 1330 || item.type == 967)
                {
                    foreach (TooltipLine tooltipLine in tooltips)
                    {
                        string resultA = tooltipLine.text.Replace("The cook might be interested in this", "Повару это должно быть интересно");
