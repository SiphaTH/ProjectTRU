...
namespace CalamityRuTranslate.Vanilla
{
    public class ManaUsageReplacements : GlobalItem
    {
        ...
        {
            if (Translation.IsRussianLanguage && ProjectTRuConfig.Instance.NewVanillaTranslation && ModLoader.GetMod("TerrariaOverhaul") == null)
            {
                foreach (TooltipLine tooltip in tooltips)
                {
                    if (tooltip.Name == "UseMana")
                    {
                        ...
                        if (int.Parse(manaUsage) == 1 || manaUsage.Length > 1 && manaUsage[manaUsage.Length - 1] == '1' && manaUsage[manaUsage.Length - 2] != '1')
                        {
